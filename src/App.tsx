...
export default function App() {
  ...
  // Trigger toast alert helper. Defensively coerces the message to a
  // plain string - if a caller ever passes an object/array (e.g. an API
  // error response shaped unexpectedly), rendering it directly as a
  // React child crashes the whole app to a blank page. This guarantees
  // that can't happen regardless of what any call site passes in.
  const triggerToast = (msg: unknown, type: 'success' | 'error' | 'warning' | 'info' = 'success') => {
    let safeMsg: string;
    if (typeof msg === 'string') {
      safeMsg = msg;
    } else if (msg == null) {
      safeMsg = 'Something went wrong.';
    } else {
      try {
        safeMsg = Array.isArray(msg)
          ? msg.map((m) => (typeof m === 'string' ? m : (m?.msg || JSON.stringify(m)))).join('; ')
          : JSON.stringify(msg);
      } catch {
        safeMsg = 'Something went wrong.';
      }
                        ...
                        return (
                          <React.Fragment key={`row-group-${rec.number}-${idx}`}>
                            ...
                            <td className={`py-4 px-4 font-mono font-bold flex items-center gap-1 ${numColorClass}`}>
                              {isStale && (
                                <span title={`Stale Pledge: Exceeds ${stalePledgeThresholdMonths}m threshold`}>

...
# Compresses responses over 1KB (JS bundle, CSS, JSON record lists) -
# cuts the ~876KB frontend bundle down to roughly 250KB over the wire.
...
async def security_headers(request: Request, call_next):
    ...
    # Vite hashes asset filenames (e.g. index-AbC123.js), so a given URL's
    # content never changes - browsers can cache these indefinitely and
    # skip re-downloading them on repeat visits.
...
async def validation_error_handler(request: Request, exc: RequestValidationError):
    # FastAPI's default `detail` is a list of error objects. The frontend
    # displays `detail` directly as toast text, and React crashes with a
    # blank page if it's ever given an object/array instead of a string -
    # so this always flattens it into one readable string first.
    messages = []
    for err in exc.errors():
        field = ".".join(str(p) for p in err.get("loc", []) if p != "body")
        messages.append(f"{field}: {err.get('msg')}" if field else err.get("msg", "Invalid input"))
...
async def update_record(
    ...
    records[index] = {
        **records[index],
        "name": payload.name,
        "phone": payload.phone if payload.phone is not None else records[index].get("phone", ""),
...
class _CachedStaticFiles(StaticFiles):
    """Vite's asset filenames include a content hash (e.g. index-Ab12Cd.js),
    so once built they never change - safe to cache aggressively, which
    means repeat visits skip re-downloading the JS/CSS bundle entirely."""
...
if os.path.isdir(_STATIC_DIR):
    app.mount("/assets", _CachedStaticFiles(directory=os.path.join(_STATIC_DIR, "assets")), name="assets")
